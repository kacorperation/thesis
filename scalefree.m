...
% m: number of nodes a new added node will be connected to such that 1 <= m <= m0
%
% OUPUT
...
[rows, columns] = find(triu(B));  %Find the upper triangular matrix values
nEdges = size(rows, 1);
...
%plot(graph(A),'layout','force','nodelabel',[])
end 